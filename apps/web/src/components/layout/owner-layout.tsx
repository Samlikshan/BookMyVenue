  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 font-sans dark:bg-zinc-950">
              ...
              <div className="flex items-center gap-3 px-2">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="truncate text-2xs text-zinc-500 dark:text-zinc-400">
                    {user?.ownerApplication?.businessName || "Venue Owner"}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-1 flex-col md:pl-64">
        ...
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-zinc-200 bg-white/80 px-6 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-900/80">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              {user?.ownerApplication?.businessName && (
          ...
          <div className="flex items-center gap-3">
            {isPending && (
              <span className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-800 dark:bg-amber-950/30 dark:text-amber-400 border border-amber-200/50 dark:border-amber-900/30">
                Pending Verification
              </span>
            )}
            {isRejected && (
              <span className="inline-flex items-center rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-semibold text-red-800 dark:bg-red-950/30 dark:text-red-400 border border-red-200/50 dark:border-red-900/30">
                Rejected
              </span>
            )}
            {user?.status === "ACTIVE" && (
              <span className="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-semibold text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-900/30">
                Verified Owner
              </span>
            )}
          </div>
        ...
        {/* Status Alerts */}
        {isPending && (
          <div className="bg-amber-500/10 px-6 py-3 border-b border-amber-500/20 text-center text-xs font-medium text-amber-800 dark:text-amber-400">
            Your owner account application is undergoing review. You can create venue drafts but cannot publish them until verified.
          </div>
        )}
        {isRejected && (
          <div className="bg-red-500/10 px-6 py-3 border-b border-red-500/20 text-center text-xs font-medium text-red-800 dark:text-red-400">
            Your owner application was rejected. Reason: {user?.ownerApplication?.rejectionReason || "Please contact support."}
          </div>
        )}
